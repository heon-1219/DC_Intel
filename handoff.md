...
- **Current state:** **M1a IN PROGRESS** — Tasks 0–8 ✅ (data layer + repo/parser + price service). **Next: Task 9** (`price_poller` region job). 48 tests pass (+2 live). Cadence: task-by-task. (M0 complete + pushed.)
...
- **M1a execution (in progress):**
  ...
  - **Next: Task 9** `price_poller` (`poll_region(db, region, redis, breaker, *, yfinance, finnhub, pykrx)` → fetch+cache each active non-index stock) → 10 `/price` → 11 scheduler (+docker smoke).
