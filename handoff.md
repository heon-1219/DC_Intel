...
- **Milestone:** M2 — Technical indicators (**M2a DONE**; **M2b NEXT**).
- **Current state:** ✅ **M2a (pure indicator engine) COMPLETE + adversarially reviewed.** `indicators.py` (RSI/EMA/MACD/Bollinger/vol_z20 + crossover + state machines + `compute_indicators` §10.1 payload) + `indicator_copy.py` (EN/KO). Every worked numeric example reproduced as the TDD oracle. **137 tests pass** (68 M1 + 69 M2a), 3 live deselected. A 6-dimension adversarial review (verifiers read the spec, not the tests) found + fixed 5 spec-faithfulness bugs (bollinger_squeeze off-by-19, stale_price on flat volume, full-precision storage, squeeze_breakout_up reachability, intraday EMA down-cross copy). Prior: 🎉 M1 FULLY COMPLETE (M1a `/price` + M1b `/prices-across-markets`, 68 tests + 3 live + docker smoke). **Next: M2b — bar fetcher (yfinance OHLCV multi-interval) + `technical_snapshots` repo + `recompute_indicators` job + scheduler wiring + docker smoke.** Cadence: stop at MILESTONES.
...
- **M2a ✅ (pure indicator engine)** — commits `83e55eb` (deps pandas+numpy) → `d016f7a` (engine: math, state machines, compute_indicators) → `f490729` (EN/KO copy) → `b1d1e20` (5 review fixes). `app/services/indicators.py` + `app/services/indicator_copy.py`. **137 tests pass.** Key implementation facts for M2b:
  ...
  - **Next: M2b** — `app/providers/yfinance_bars.py` (`fetch_bars(ref, interval)` → OHLCV DataFrame, lazy yfinance, `prepost=False`, drop incomplete last bar, ProviderError + `@pytest.mark.live`), `app/db/repositories/technical_snapshots.py` (upsert mapping scalar cols `rsi/ema_*/macd*/bollinger_*` from payload + `indicators_json`; `get_latest`), `app/services/indicator_pipeline.py` (per active stock × interval: fetch→compute→persist), `app/jobs/indicator_calculator.py` (`recompute_indicators`), scheduler/lifespan wiring (5-min cadence), docker smoke. Write the M2b plan first, then execute.
