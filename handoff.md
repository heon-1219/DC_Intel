...
- **M1b ✅ (code)** — T1 FX provider (`fx_provider.py` USDKRW via yfinance) + `services/fx.get_usdkrw` (cache 5min); T2 `StockRef` names + `repo.get_company_listings` (Listing); T3 `services/xmkt.build_cross_market` (per-underlying-share USD via FX/ADR-ratio, diff_pct_vs_base, missing→null); T4 `GET /stocks/{i}/prices-across-markets` + `/price` real names. Commits `c116037`→`69b5f37`. **68 pass + 3 live.**
  - **🎉 M1 COMPLETE (code).** Only the docker smoke is outstanding (blocked on Docker Desktop — see TO RUN above). **Next milestone: M2 — technical indicators** (RSI/EMA/MACD/Bollinger/vol_z20 → technical_snapshots → indicator_calculator job; oracle = the worked numeric examples in technical-indicators.md). Write the M2 plan first, then execute.
